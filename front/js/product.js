...
fetch("http://localhost:3000/api/products")
    ...
    .then((data) => {
        const product = data.find((item) => item._id === url);
        if (product) {
            ...
            let affichageDescription = document.createTextNode(product.description);
            ...
            for (let c = 0; c < product.colors.length; c++) {
                ...
                optionProduit.value = product.colors[c];
                let affichageTxtProduit = document.createTextNode(product.colors[c]);
